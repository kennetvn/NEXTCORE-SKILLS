 ...
 *
 * Fires when Plan subagent completes. Reminds to invoke /nc:cook --auto before implementation.
...
try {
  ...
  const { isHookEnabled, readSessionState } = require('./lib/nc-config-utils.cjs');
  ...
  try {
    ...
    console.log('MUST invoke /nc:cook --auto skill before implementing the plan');
    if (planPath) {
      const planMdPath = path.join(planPath, 'plan.md');
      console.log(`Best Practice: Run /clear then /nc:cook ${planMdPath}`);
    } else {
      // Fallback when plan path unavailable
      console.log('Best Practice: Run /clear then /nc:cook {full-absolute-path-to-plan.md}');
    }
