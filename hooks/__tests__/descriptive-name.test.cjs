...
describe('descriptive-name.cjs', () => {
  ...
  describe('Hook disable functionality', () => {
    ...
    it('exits silently when disabled via .nc.json', async () => {
      // Create .claude/.nc.json with hook disabled
      const claudeDir = path.join(tempDir, '.claude');
      fs.mkdirSync(claudeDir, { recursive: true });
      fs.writeFileSync(
        path.join(claudeDir, '.nc.json'),
      ...
      fs.copyFileSync(
        path.join(__dirname, '..', 'lib', 'nc-config-utils.cjs'),
        path.join(libDir, 'nc-config-utils.cjs')
      );
