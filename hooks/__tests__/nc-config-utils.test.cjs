#!/usr/bin/env node
/**
 * Tests for nc-config-utils.cjs - Config and path utilities
 * Run: node --test .claude/hooks/__tests__/nc-config-utils.test.cjs
 *
 * Covers:
...
const {
  ...
  getGitRoot,
  DEFAULT_CONFIG
} = require('../lib/nc-config-utils.cjs');

describe('nc-config-utils.cjs', () => {
