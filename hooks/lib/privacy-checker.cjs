...
/**
 * Load .nc.json config to check if privacy block is disabled
 * @param {string} [configDir] - Directory containing .nc.json (defaults to .claude in cwd)
 * @returns {boolean} true if privacy block should be skipped
 */
function isPrivacyBlockDisabled(configDir) {
  try {
    const configPath = configDir
      ? path.join(configDir, '.nc.json')
      : path.join(process.cwd(), '.claude', '.nc.json');
 ...
 * @param {string} [params.options.configDir] - Directory for .nc.json config
