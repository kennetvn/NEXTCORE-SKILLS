/**
 * Shared utilities for NextCoreSkill hooks
 *
...
const LOCAL_CONFIG_PATH = '.claude/.nc.json';
const GLOBAL_CONFIG_PATH = path.join(os.homedir(), '.claude', '.nc.json');
 ...
 * @param {Object} config - NextCoreSkill config
 ...
 *   2. Global config (~/.claude/.nc.json) - user preferences
 *   3. Local config (./.claude/.nc.json) - project-specific overrides
 *
