...
const { loadConfig } = require('./hooks/lib/nc-config-utils.cjs');
 ...
 * Optional inactivity timeout can be enabled via NC_STATUSLINE_STDIN_TIMEOUT_MS.
 */
async function readStdin() {
  return new Promise((resolve, reject) => {
    ...
    const parsedTimeout = Number.parseInt(env.NC_STATUSLINE_STDIN_TIMEOUT_MS || '', 10);
...
async function main() {
  try {
    ...
    try {
      ...
      if (sessionId) {
        const sessionPath = path.join(os.tmpdir(), `nc-session-${sessionId}.json`);
