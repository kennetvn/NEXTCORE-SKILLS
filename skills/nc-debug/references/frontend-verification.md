...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `nc:chrome-devtools` skill fallback.
...
Check if Chrome MCP server is available via `nc:mcp-management` skill or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, use `nc:chrome-devtools` skill (Puppeteer with bundled Chromium):
...
If `nc:chrome-devtools` skill is also unavailable, skip visual verification and note in report:
