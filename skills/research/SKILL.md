...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.claude/.nc.json` (or `~/.claude/.nc.json`) for `skills.research.useGemini` (default: `true`). If `false`, skip Gemini and use WebSearch.
   - **Gemini Model**: Read from `.claude/.nc.json`: `gemini.model` (default: `gemini-3-flash-preview`)
...
2. **Deep Content Analysis**:
   - When you found a potential Github repository URL, use `nc:docs-seeker` skill to find read it.
...
## Output Requirements
**IMPORTANT:** Invoke "/nc:project-organization" skill to organize the outputs.
