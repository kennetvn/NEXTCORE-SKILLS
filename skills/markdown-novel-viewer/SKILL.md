...
**Note:** HTML generation mode (`/nc:preview --html ...`) produces self-contained HTML files that open directly in the browser — they do not use this server.
...
```bash
# Option 1: Install via NextCoreSkill CLI (recommended)
...
Use `/nc:preview` for quick access:

```bash
/nc:preview plans/my-plan/plan.md    # View markdown file
/nc:preview plans/                   # Browse directory
/nc:preview --stop                   # Stop server
```
