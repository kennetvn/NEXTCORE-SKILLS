...
Stitch SDK has no programmatic quota check endpoint. NextCoreSkill tracks locally:

**File:** `~/.nextcoreskill/.stitch-quota.json`
...
2. Skill prints: "Daily quota exhausted. Use nc:ui-ux-pro-max as fallback."
3. Activate `nc:ui-ux-pro-max` with the same design prompt
...
Local tracking can drift if user generates designs outside NextCoreSkill (via Stitch web UI or other tools). If you hit `RATE_LIMITED` error despite local tracker showing credits available:
