...
Orchestration workflows chaining Stitch design generation with NextCoreSkill implementation skills.
...
# 4. Activate nc:frontend-design to implement React components from the spec
```
...
|-------------|-------|-------------|
| React/Vue/Svelte components | `nc:frontend-design` | Converts Tailwind HTML to framework components |
| Full page with style guide | `nc:ui-ux-pro-max` | Builds complete layouts with design system |
| Design token extraction | `nc:ui-styling` | Extracts shadcn/Tailwind tokens from DESIGN.md |

## Handoff Protocol

1. `nc:stitch` exports artifacts to `./stitch-exports/` (or plan directory)
...
# -> Skip Stitch, use nc:ui-ux-pro-max for text-based design instead
