...
1. **Check quota** — Run `stitch-quota.ts check`. If exhausted, suggest `nc:ui-ux-pro-max` fallback.
...
6. **Implement** — Hand off exported artifacts to implementation skill:
   - `nc:frontend-design` — React/Vue/Svelte components from Tailwind export
   - `nc:ui-ux-pro-max` — Full page layouts with style guide integration
   - `nc:ui-styling` — Design token extraction from DESIGN.md
...
- Local tracking via `~/.nextcoreskill/.stitch-quota.json`
- Warns when remaining credits < 20%
- **Fallback:** When exhausted, use `nc:ui-ux-pro-max` for text-based design generation
