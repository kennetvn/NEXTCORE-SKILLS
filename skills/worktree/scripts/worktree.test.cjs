...
const MONOREPO_DIR = '/home/kai/nextcoreskill';
...
test('create in monorepo with project works', () => {
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'nextcoreskill-engineer', 'Should detect project');
});
...
test('superproject detection in submodule', () => {
  // Test from nextcoreskill-engineer submodule
  const submodulePath = '/home/kai/nextcoreskill/nextcoreskill-engineer';
...
test('create with partial project match in monorepo', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'cli' should match 'nextcoreskill-cli'
  ...
  const json = assertJSON(result.output);
  assert(json.wouldCreate.project === 'nextcoreskill-cli', 'Should find nextcoreskill-cli');
});

test('create detects multiple project matches', () => {
  if (!fs.existsSync(MONOREPO_DIR)) return;
  // 'nextcoreskill' matches multiple projects
  const result = run('create nextcoreskill test-multi --prefix feat --json', { cwd: MONOREPO_DIR });
