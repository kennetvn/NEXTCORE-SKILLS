...
```
/nc:team <template> <context> [flags]
```

**Templates:** `nc:research`, `nc:cook`, `nc:code-review`, `nc:debug`
...
4. Do NOT fall back to subagents. `/nc:team` MUST use Agent Teams or abort.
...
## ON `/nc:team research <topic>` [--researchers N]:

*Wraps /nc:research skill -- scope, gather, analyze, report.*
...
11. **JOURNAL**: Run `/nc:journal` to write a concise technical journal entry upon completion

---

## ON `/nc:team cook <plan-path-or-description>` [--devs N]:

*Wraps /nc:cook skill -- plan, code, test, review, finalize.*
...
7. **DOCS SYNC EVAL** (MANDATORY for cook -- from /nc:cook finalize):
   ```
...
## ON `/nc:team review <scope>` [--reviewers N]:

*Wraps /nc:code-review skill -- scout, review, synthesize with evidence gates.*
...
## ON `/nc:team debug <issue>` [--debuggers N]:

*Wraps /nc:fix skill -- root-cause-first, adversarial hypotheses, disprove to converge.*
