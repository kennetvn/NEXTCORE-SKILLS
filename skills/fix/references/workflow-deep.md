...
Activate `nc:debug` skill. Launch 2-3 `Explore` subagents in parallel:
```
...
- Activate `nc:project-management` for task sync-back, plan status updates, and progress tracking
...
|------|------------------|
| 1 | `nc:debug`, parallel `Explore` subagents for investigation |
...
**Visual Assets:** Use `nc:ai-multimodal` for visual assets generation, analysis and verification.
