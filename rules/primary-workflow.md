...
- Use `/nc:preview --explain <topic>` to generate visual explanation with ASCII + Mermaid
- Use `/nc:preview --diagram <topic>` for architecture and data flow diagrams
- Use `/nc:preview --slides <topic>` for step-by-step walkthroughs
- Use `/nc:preview --ascii <topic>` for terminal-friendly output only
- **HTML mode** (add `--html` for self-contained HTML pages, opens directly in browser):
  - `/nc:preview --html --explain <topic>` — publication-quality HTML explanation
  - `/nc:preview --html --diagram <topic>` — interactive HTML diagram with zoom controls
  - `/nc:preview --html --slides <topic>` — magazine-quality slide deck
  - `/nc:preview --html --diff [ref]` — visual diff review
  - `/nc:preview --html --plan-review` — plan vs codebase comparison
