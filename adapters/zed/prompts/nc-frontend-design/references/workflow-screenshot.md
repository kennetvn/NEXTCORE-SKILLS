...
## Prerequisites
- follow the `nc-ui-ux-pro-max` workflow first for design intelligence
- Have `nc-ai-multimodal` workflow ready for visual analysis

## Workflow Steps

### 1. Analyze Screenshot Details
follow the `nc-ai-multimodal` workflow to extract:
...
### 4. Generate Assets
follow the `nc-ai-multimodal` workflow:
- Generate images, icons, backgrounds
- Verify generated assets match design
- Remove backgrounds if needed with `nc-media-processing`
