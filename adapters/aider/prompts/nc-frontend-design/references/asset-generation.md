...
Complete workflow for generating design-aligned visual assets using `nc-ai-multimodal` workflow.
...
follow the `nc-ai-multimodal` workflow's image generation capabilities:
...
Use `nc-ai-multimodal` to analyze the generated asset (see `visual-analysis.md` for complete workflow):
...
4. Consider using `nc-media-processing` workflow for post-generation adjustments
