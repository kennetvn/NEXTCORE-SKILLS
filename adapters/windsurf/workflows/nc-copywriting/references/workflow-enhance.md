...
1. **Screenshots provided** → follow the `nc-ai-multimodal` workflow to analyze and describe issues in detail
2. **Videos provided** → Use `nc-ai-multimodal` video-analysis to extract relevant copy issues
3. **Scout codebase** → `/nc-scout ext` (preferred) or `/nc-scout` to find files needing enhancement
