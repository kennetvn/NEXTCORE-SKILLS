...
Use chat question to probe user's request, constraints, true objectives.
...
3. Present 2-3 options with pros/cons via chat question
...
2. Use `ui-ux-designer` + `researcher` subagents in parallel:
   ...
   - Describe assets for `nc-ai-multimodal` workflow generation
3. `ui-ux-designer` creates:
   - Design guidelines at `./docs/design-guidelines.md`
   - Wireframes in HTML at `./docs/wireframe/`
4. If no logo provided: generate with `nc-ai-multimodal` workflow
5. Screenshot wireframes with `nc-chrome-devtools` → save to `./docs/wireframes/`

**Gate:** User approves design. Repeat if rejected.

**Image tools:** `nc-ai-multimodal` for generation/analysis, `imagemagick` for crop/resize, background removal tool as needed.

## Step 5: Planning

Activate **nc-plan** skill: `/nc-plan --hard <requirements>`
...
Activate **nc-cook** skill: `/nc-cook <plan-path>` (interactive mode — review gates at each step)
