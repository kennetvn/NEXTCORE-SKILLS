...
2. Determine ship mode from arguments:
   ...
   - No argument → infer from branch name:
     ...
     - Unclear → chat question with options: "Official (main)", "Beta (dev)"
...
- **If no test runner detected:** Use chat question — "No test runner detected. Skip tests or provide command?"
...
5. **If critical issues found:** For EACH critical issue, use chat question:
...
3. Auto-decide bump level from diff size:
   ...
   - **Major feature or breaking change:** Use chat question — "This looks like a significant change. Bump minor or patch?"
...
1. follow the `nc-journal` workflow via `journal-writer` subagent in background:
...
1. Invoke `/nc-docs update` skill via `docs-manager` subagent in background:
