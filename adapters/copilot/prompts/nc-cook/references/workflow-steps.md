...
- Use `/nc-scout ext` or `scout` agent for codebase search
...
- Use chat question to ask: "Proceed to planning?" / "Request more research" / "Abort"
...
**Fast:**
- Use `/nc-plan --fast` with scout results only
- Minimal planning, focus on action

**Parallel:**
- Use `/nc-plan --parallel` for dependency graph + file ownership matrix
...
- Use chat question to ask: "Validate the plan or approve plan to start implementation?" - "Validate" / "Approve" / "Abort" / "Other" ("Request revisions")
  - "Validate": run `/nc-plan validate` skill invocation
...
- Use `nc-ai-multimodal` for image assets
...
- Use chat question to ask: "Proceed to testing?" / "Request implementation changes" / "Abort"
- **Auto mode:** Skip this gate
...
- Use chat question to ask: "Proceed to code review?" / "Request test fixes" / "Abort"
- **Auto mode:** Skip this gate
