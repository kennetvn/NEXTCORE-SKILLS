...
## Prerequisites
- follow the `nc-ui-ux-pro-max` workflow first
- follow the `nc-threejs` workflow for 3D and WebGL expertise
- Have `nc-ai-multimodal` workflow ready for asset generation

## Initial Research
Run `nc-ui-ux-pro-max` searches:
```bash
...
follow the `nc-ai-multimodal` workflow for:
- Textures and materials
- Skyboxes and environment maps
- Particle sprites
- Video backgrounds

follow the `nc-media-processing` workflow for:
