...
1. `nc-ui-ux-pro-max` - Design database (ALWAYS FIRST)
2. `nc-ui-ux-pro-max` - Design principles
3. `nc-frontend-design` - Implementation patterns
...
Analyze screenshots/videos with `nc-ai-multimodal` workflow.
...
Screenshot + `nc-ai-multimodal` analysis.
...
follow the `nc-chrome-devtools` workflow.
...
## Tips
- Use `nc-ai-multimodal` for generating visual assets
