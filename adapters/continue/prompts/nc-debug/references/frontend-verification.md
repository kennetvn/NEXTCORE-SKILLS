...
Visual verification of frontend implementations using Chrome MCP (Claude Chrome Extension) or `nc-chrome-devtools` workflow fallback.
...
Check if Chrome MCP server is available via `nc-mcp-management` workflow or `ListMcpResourcesTool`:
...
When Chrome MCP is not configured, follow the `nc-chrome-devtools` workflow (Puppeteer with bundled Chromium):
...
If `nc-chrome-devtools` workflow is also unavailable, skip visual verification and note in report:
