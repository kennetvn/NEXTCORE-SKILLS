...
Entry point for using the `nc-ai-multimodal` workflow to generate and analyze visual assets that align with frontend design thinking and aesthetic guidelines.

## When to Use

Use `nc-ai-multimodal` in frontend design when you need to:
...
Use `aesthetic` for overall design system guidance and quality evaluation framework. Then use `nc-frontend-design` with `nc-ai-multimodal` for asset generation and analysis that follows those guidelines.

### With `nc-chrome-devtools` Skill
Use `nc-chrome-devtools` to capture full-screen screenshots from inspiration websites for design extraction. Capture at actual viewport size, not full-page scrolls.
...
### With `nc-media-processing` Skill
